...
function createHtml() {
    const newHTML = inputQueue
        .map((input, i) => {
            return `<div class="is-level"><span class="tag is-success is-large">${input.pieceSize} cm x ${input.pieceAmount} kom<button class="delete" onClick="removeInput(${i})"></button></span></div>`
        })
        .join("")
